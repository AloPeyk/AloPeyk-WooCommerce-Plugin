...
if ( $order_data ) {
	...
	$address_delimiter = __( ',', 'alopeyk-woocommerce-shipping' ) . ' ';
?>
<ul>
	<li class="wide awcshm-meta-box-content-container">
		...
		<div class="postbox awcshm-address-info-box">
			<div class="awcshm-address-info-title">
				<span><?php echo __( 'Order Created', 'alopeyk-woocommerce-shipping' ); ?></span>
		...
		<div class="postbox awcshm-address-info-box">
			<div class="awcshm-address-info-title">
				<span><?php echo __( 'Order Scheduled', 'alopeyk-woocommerce-shipping' ); ?></span>
		...
		<div class="postbox awcshm-address-info-box">
			<div class="awcshm-address-info-title">
				<span><?php echo __( 'Order Accepted', 'alopeyk-woocommerce-shipping' ); ?></span>
		...
		<div class="postbox awcshm-address-info-box">
			<div class="awcshm-address-info-title">
				...
				<span><?php echo sprintf( ( $address->type == 'return' ? __( 'Courier returned to %s.', 'alopeyk-woocommerce-shipping' ) : __( 'Courier reached %s.', 'alopeyk-woocommerce-shipping' ) ), ( in_array( $address->type, array( 'origin', 'return' ) ) ? __( 'origin', 'alopeyk-woocommerce-shipping' ) : __( 'destination', 'alopeyk-woocommerce-shipping' ) . ( count( $order_data->addresses ) < 3 || ( count( $order_data->addresses ) == 3 && $order_data->has_return ) ? '' : ' ' . $key ) ) ); ?></span>
				<span><?php echo date_i18n( 'j F Y (g:i A)', strtotime( $address->handled_at ) ) ?></span>
				<?php } else { ?>
				<span><?php echo in_array( $address->type, array( 'origin', 'return' ) ) ? __( 'origin', 'alopeyk-woocommerce-shipping' ) : __( 'destination', 'alopeyk-woocommerce-shipping' ) . ( count( $order_data->addresses ) < 3 || ( count( $order_data->addresses ) == 3 && $order_data->has_return ) ? '' : ' ' . $key ); ?></span>
				<?php } ?>
			</div>
			<div class="awcshm-address-info-inside">
				<ul class="awcshm-address-info-items">
					...
					<li>
						<strong><?php echo __( 'Address', 'alopeyk-woocommerce-shipping' ); ?>: </strong>
						<?php
							...
							if ( ! empty( $address->unit ) ) {
								$address_parts[] = __( 'Unit', 'alopeyk-woocommerce-shipping' ) . ' ' . $address->unit;
							}
							if ( ! empty( $address->number ) ) {
								$address_parts[] = __( 'Plaque', 'alopeyk-woocommerce-shipping' ) . ' ' . $address->number;
							}
						?>
					...
					<li>
						<strong><?php echo __( 'Description', 'alopeyk-woocommerce-shipping' ); ?>: </strong>
						<span><?php echo str_replace( 'nn','<br>',$address->description ); ?></span>
					</li>
					<?php } ?>
					<?php if ( ! empty( $address->person_fullname ) ) { ?>
					<li>
						<strong><?php echo __( 'Name', 'alopeyk-woocommerce-shipping' ); ?>: </strong>
						<span><?php echo $address->person_fullname; ?></span>
					</li>
					<?php } ?>
					<?php if ( ! empty( $address->person_phone ) ) { ?>
					<li>
						<strong><?php echo __( 'Phone', 'alopeyk-woocommerce-shipping' ); ?>: </strong>
						<span><?php echo $address->person_phone; ?></span>
					</li>
					<?php } ?>
					<?php if ( ! empty( $address->signed_by ) ) { ?>
					<li>
						<strong><?php echo __( 'Signed By', 'alopeyk-woocommerce-shipping' ); ?>: </strong>
				...
				<div align="center">
					<img src="<?php echo $helpers->get_signature_url( $address->signature->url ); ?>" alt="<?php echo __( 'Signature', 'alopeyk-woocommerce-shipping' ); ?>" title="<?php echo $address->signed_by; ?>" class="awcshm-address-signature">
				</div>
				<?php } ?>
			</div>
		</div>
		...
		<div class="postbox awcshm-address-info-box">
			<div class="awcshm-address-info-title">
				<span><?php echo __( 'Order Deleted', 'alopeyk-woocommerce-shipping' ); ?></span>
		...
		<div class="postbox awcshm-address-info-box">
			<div class="awcshm-address-info-title">
				<span><?php echo __( 'Order Canceled', 'alopeyk-woocommerce-shipping' ); ?></span>
				<span><?php echo date_i18n( 'j F Y (g:i A)', strtotime( $order_data->updated_at ) ) ?></span>
			</div>
		</div>
		<?php } ?>
		...
		<div class="postbox awcshm-address-info-box">
			<div class="awcshm-address-info-title">
				<span><?php echo __( 'Order Expired', 'alopeyk-woocommerce-shipping' ); ?></span>
				<span><?php echo date_i18n( 'j F Y (g:i A)', strtotime( $order_data->updated_at ) ) ?></span>
			</div>
		</div>
		<?php } ?>
