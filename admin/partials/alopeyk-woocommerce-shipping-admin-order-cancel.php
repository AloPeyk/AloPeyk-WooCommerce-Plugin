...
$order_label = $order_data ? '#' . $order_data->invoice_number : ( $order_id ? $order_id : esc_html__( 'Alopeyk order', 'alopeyk-shipping' ) );
...
<form class="awcshm-cancel-order-form">
	...
	<?php
		echo wp_kses(
			sprintf(
				...
				html_entity_decode(esc_html__('You are about to cancel <strong><a href="%1$s" target="_blank">%2$s</a></strong>.', 'alopeyk-shipping')),
				esc_url($edit_link),
				$order_label
			),
			array(
	...
	<div class="error notice">
		...
		<p><?php echo sprintf( esc_html__('You will be charged %1$s for this cancellation, because more than %2$s minutes passed after order being accepted by Alopeyk courier.', 'alopeyk-shipping'), esc_html(wc_price($penalty_amount)), esc_html($penalty_delay)); ?></p>	
		<?php } else { ?>
		<?php /* translators: %s: First: money */ ?>
		<p><?php echo sprintf( esc_html__('You will be charged %s for this cancellation.', 'alopeyk-shipping'), esc_html(wc_price($penalty_amount))); ?></p>
		<?php } ?>
	</div>
	<?php
		}
	...
	<div class="awcshm-radio-list-container">
		<p>
			<strong><?php echo esc_html__( 'Which of the following reasons make you decide to cancel this request?', 'alopeyk-shipping' ); ?></strong>
		</p>
		<p>
